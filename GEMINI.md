...
- Duration-based uptime calculation (accurate tracking for zoomed-in historical outage windows)
- Intelligent Y-axis optimization (Uptime Kuma-inspired)
- Dark/Light theme support with instant switching
- Advanced mobile optimizations (Chart.js LTTB Decimation, parsing bypass for extreme 90d/1y performance)
...
#### Custom Zoom Functionality (monitoring.html Lines 1620-2450)

The application implements custom zoom **without** Chart.js zoom plugin.

##### Zoom Mode Toggle (Global / Individual)

A toggle button in the header switches between two zoom modes, persisted in `localStorage`:

**HTML Button (Lines 1620-1625):**
```html
<button id="zoomModeToggle"
    class="btn bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg flex items-center gap-2"
    title="Prepnúť režim zoom-out (Individuálny / Globálny)" onclick="toggleZoomMode()">
    <i class="fas fa-fw fa-compress-arrows-alt text-base" id="zoomModeIcon"></i>
    <span id="zoomModeText">Individuálny</span>
</button>
```

**State Variable:**
```javascript
// Loaded from localStorage on init, defaults to false (Individual)
window._globalZoomMode = localStorage.getItem('mikrotik-zoom-mode') === 'global';
```

**Toggle Function:**
```javascript
function toggleZoomMode() {
    window._globalZoomMode = !window._globalZoomMode;
    localStorage.setItem('mikrotik-zoom-mode', window._globalZoomMode ? 'global' : 'individual');
    applyZoomModeUI();
    addDebugLog(`🔀 Zoom mode: ${window._globalZoomMode ? 'GLOBÁLNY' : 'INDIVIDUÁLNY'}`);
}

function applyZoomModeUI() {
    const btn = document.getElementById('zoomModeToggle');
    const icon = document.getElementById('zoomModeIcon');
    const text = document.getElementById('zoomModeText');
    if (window._globalZoomMode) {
        btn.classList.add('zoom-global-active');       // Orange color (#d97706)
        icon.className = 'fas fa-fw fa-expand-arrows-alt text-base';
        if (text) text.textContent = 'Globálny';
        btn.title = 'Zoom: Globálny režim – prepnúť na Individuálny';
    } else {
        btn.classList.remove('zoom-global-active');    // Gray (default)
        icon.className = 'fas fa-fw fa-compress-arrows-alt text-base';
        if (text) text.textContent = 'Individuálny';
        btn.title = 'Zoom: Individuálny režim – prepnúť na Globálny';
    }
}
```
...
| Feature | Individuálny (default) | Globálny |
|---------|----------------------|---------|
| Drag zoom | Affects clicked chart only | Applies same range to ALL 4 charts |
| Zoom-out button | Clicked chart only | All 4 charts simultaneously |
| Double-click | Clicked chart only | All 4 charts simultaneously |
| Double-tap (mobile) | Clicked chart only | All 4 charts simultaneously |
| Button color | Gray | Orange (`#d97706`) |
| Icon | `fa-compress-arrows-alt` | `fa-expand-arrows-alt` |
| Mobile text | Hidden (icon only) | Hidden (icon only) |
| Persistence | `localStorage` | `localStorage` |

**CSS Styling (Lines 927-982):**
```css
/* Fixed-width label – prevents layout shift on mode toggle */
#zoomModeText { display: inline-block; min-width: 6.2em; text-align: left; }

/* Global mode: orange */
#zoomModeToggle.zoom-global-active { background-color: #d97706 !important; }

/* Light theme variant */
body.light-theme #zoomModeToggle.zoom-global-active { background-color: #f59e0b !important; }

/* Mobile: text hidden */
@media (max-width: 480px) { #zoomModeText { display: none !important; } }
```

**Button Click Handler with Rate Limiting:**
```javascript
function resetZoomButton(chartId) {
    // Rate limiting: 500ms minimum between clicks (per-chart)
    if (!window._buttonClickTimes) window._buttonClickTimes = {};
    const now = Date.now();
    if ((now - (window._buttonClickTimes[chartId] || 0)) < 500) return;
    window._buttonClickTimes[chartId] = now;

    if (window._globalZoomMode) {
        // GLOBAL: zoom-out all charts
        ['pingChart', 'cpuChart', 'temperatureChart', 'memoryChart'].forEach(id => resetZoom(id));
    } else {
        // INDIVIDUAL: zoom-out clicked chart only
        resetZoom(chartId);
    }
}
```
...
|----------|------|---------|
| `window._globalZoomMode` | Boolean | `true` = Globálny, `false` = Individuálny |
| `window._chartTimeRanges` | `Map<string, string>` | Per-chart time range (e.g. `pingChart → "7d"`) |
| `window._expandedCharts` | `Set<string>` | Charts permanently expanded to larger time range |
| `window._wasExpanded` | `Set<string>` | Cleanup tracking for expanded charts |
| `window._buttonClickTimes` | `Object` | Rate-limit timestamps per chart (500ms debounce) |
| `window._lastZoomResetTime` | `Number` | Global debounce for resetZoom (400ms) |

**localStorage Keys:**

| Key | Values | Default |
|-----|--------|---------|
| `mikrotik-zoom-mode` | `'global'` / `'individual'` | `'individual'` |
...
**Lines of Code:** ~8542 total
- monitoring.html: ~4623 lines
- monitoring.js: ~3919 lines
...
2. **Global / Individual zoom mode toggle** (synchronized or per-chart zoom)
3. Per-chart independent time ranges (`window._chartTimeRanges` Map)
4. Permanently expanded chart state (`window._expandedCharts` Set)
5. Optimized refresh (partial + full load)
6. Smart Y-axis positioning algorithms
7. Intelligent SNMP gap detection
8. Duration-based uptime/latency calculation
9. Forward-fill memory to prevent truncation
10. Complete dark/light theme system
11. Advanced mobile optimizations
12. Rate limiting on zoom buttons (500ms debounce per chart)

**Planned / In Progress:**
- Chart rendering optimization (reduce unnecessary redraws, improve large dataset handling)
...
**Version**: Based on app.py analysis, manual.md documentation, and comprehensive monitoring system analysis (incl. Global/Individual zoom mode system)
