...
def backup_mikrotik(mikrotik_info):
    ...
    try:
        ...
        backup_file = f"{safe_identity}_{mikrotik_info['ip']}_{date}.backup"
        ...
        sftp.get(rsc_file, os.path.join(LOCAL_DIRECTORY, rsc_file))
        
        # Stiahnutie .backup suboru
        sftp.get(backup_file, os.path.join(LOCAL_DIRECTORY, backup_file))
...
if __name__ == "__main__":
    for mikrotik_info in MIKROTIK_INFO:
        ...
        if rsc_file:
            ...
            upload_to_ftp(f'{LOCAL_DIRECTORY}/{rsc_file}', rsc_file)
            
            # Nahrat .backup subor na FTP
            upload_to_ftp(f'{LOCAL_DIRECTORY}/{backup_file}', backup_file)
